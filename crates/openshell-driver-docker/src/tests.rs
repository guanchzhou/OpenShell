...
async fn tracing_standalone_rpc_layer_propagates_context_and_records_errors() {
    ...
    assert!(capabilities.attributes.iter().any(|attribute| {
        attribute.key.as_str() == "rpc.service"
            && attribute.value.to_string() == "openshell.compute.v1.ComputeDriver"
    }));
    assert!(capabilities.attributes.iter().any(|attribute| {
        attribute.key.as_str() == "rpc.method" && attribute.value.to_string() == "GetCapabilities"
    }));
    let failed = spans
        .iter()
